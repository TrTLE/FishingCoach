...
class FishDb(private val dbHelper: FishDbHelper = FishDbHelper.instance) {
    ...
    fun requestFishNameById(fishID: Int) = dbHelper.use {
        ...
        val cursor =
            query(FishTable.NAME, projections, selection, selectionArg, null, null, null, null)
    ...
    fun requestFishByPlace(place: String) = dbHelper.use {
        val rawQuery =
            "SELECT F.* FROM FISH as F, FISHING_DETAILS as D, LIVINGPLACE as L WHERE F.ID = D.FISH AND L.name = '" + place + "' AND D.PLACE = L.ID GROUP BY F.name;"
        val cursor = rawQuery(rawQuery, null)
    ...
    fun requestFishingTypeFromFishAndPlace(fishName: String, place: String) = dbHelper.use {
        val rawQuery =
            "SELECT T.* FROM FISH as F, FISHING_DETAILS as D, LIVINGPLACE as L, FISHINGTYPE as T WHERE F.ID = D.FISH AND D.PLACE = L.ID AND D.FISHINGTYPE = T.ID AND F.NAME ='" + fishName + "' AND L.name ='" + place + "' GROUP BY D.FISHINGTYPE;"
        val cursor = rawQuery(rawQuery, null)
    ...
    fun requestGetMaterialByFishId(fishID: Int) = dbHelper.use {
        val rawQuery =
            "SELECT M.* FROM MATERIAL as M, FISHING_DETAILS as F WHERE M.ID = F.MATERIAL AND F.FISH = '" + fishID + "';"
        val cursor = rawQuery(rawQuery, null)
        cursor.parseList(classParser<Material>())
    }

    fun requestGetMaterialByFishIdAndFishingTypeID(fishID: Int, fishingTypeID: Int, placeID: Int) =
        dbHelper.use {
            val rawQuery =
                "SELECT M.* FROM FISH as F, FISHING_DETAILS as D, LIVINGPLACE as L, FISHINGTYPE as T, MATERIAL as M WHERE F.ID = D.FISH AND D.PLACE = L.ID AND D.FISHINGTYPE = T.ID AND D.MATERIAL = M.ID AND F.ID ='" + fishID + "' AND T.ID ='" + fishingTypeID + "' AND L.ID ='" + placeID + "';"
            val cursor = rawQuery(rawQuery, null)
            cursor.parseList(classParser<Material>())
        }
}